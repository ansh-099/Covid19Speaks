...
const LikedScreen = () => {
  ...
  const response = await djangoapi.post('/api/all/getrangedunitslikes/',{'start': state.start, 'end': state.start+ 10},{headers: {
  ...
  return (
      ...
      <Header
        ...
        centerComponent={{ text: 'Most Liked' , style: styles.writing}}
        />
        <FlatList
        onEndReached={() => {
          add_elements();
...
LikedScreen.navigationOptions = {
  tabBarIcon: <MaterialIcons name="pageview" size={28} color="black" />
